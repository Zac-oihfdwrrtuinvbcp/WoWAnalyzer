...
class DanceDamageTracker extends FilteredDamageTracker {
  ...
  shouldProcessEvent(event: any): boolean {
    ...
    //kept opportunity stacks instead of "hasBuff" in case rule changes in the future
    ...
    const isVanishReady = this.spellUsable.isOnCooldown(SPELLS.VANISH.id);

    return (opportunityStacks == 0 ) && !hasAudacity && !isInStealthState && !isVanishReady;
  }
}
